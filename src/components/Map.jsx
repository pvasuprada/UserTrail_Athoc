...
const ZOOM_THRESHOLD = 14; // Zoom level threshold for grouping
...
const ArcGISMap = () => {
  ...
  const updateMarkerVisibility = (view, zoom) => {
    console.log("Current zoom level:", zoom);
    const showGrouped = zoom < ZOOM_THRESHOLD;
    console.log("Should show grouped markers:", showGrouped);

    // Get all individual markers and their labels
    const individualMarkers = view.graphics.filter(
      (g) => g.attributes?.user && !g.attributes?.isGroupMarker
    );
    console.log(
      "Individual markers:",
      individualMarkers.map((m) => m.attributes.user)
    );
    console.log("Number of individual markers:", individualMarkers.length);

    const labelMarkers = view.graphics.filter((g) => g.symbol?.type === "text");
    console.log(
      "Label markers:",
      labelMarkers.map((m) => m.symbol.text)
    );
    console.log("Number of label markers:", labelMarkers.length);

    // Get or create group marker
    let groupMarker = view.graphics.find((g) => g.attributes?.isGroupMarker);
    console.log("Existing group marker:", groupMarker ? "yes" : "no");

    if (showGrouped) {
      // Hide individual markers and labels
      individualMarkers.forEach((m) => {
        m.visible = false;
        console.log(`Hiding individual marker for user ${m.attributes.user}`);
      });
      labelMarkers.forEach((m) => {
        m.visible = false;
        console.log(`Hiding label marker ${m.symbol.text}`);
      });

      if (!groupMarker) {
        // Create group marker if it doesn't exist
        const userLocations = {};
        ...
        console.log("Group marker attributes:", groupMarker.attributes);
        view.graphics.add(groupMarker);
      }

      console.log("Setting group marker visible");
      groupMarker.visible = true;
    } else {
      console.log("Showing individual markers");
      // Show individual markers and labels
      individualMarkers.forEach((m) => (m.visible = true));
      labelMarkers.forEach((m) => (m.visible = true));

      // Hide group marker
      if (groupMarker) {
        console.log("Hiding group marker");
        groupMarker.visible = false;
      }
    }
  };

  // Function to create group marker symbol
  const createGroupMarkerSymbol = () => {
    ...
    console.log("Created group marker SVG URL:", svgUrl);
    return svgUrl;
  };
  ...
  useEffect(() => {
    if (mapDiv.current) {
      ...
      view.watch("zoom", (newZoom) => {
        ...
        updateMarkerVisibility(view, newZoom);
        setMapStatus((prev) => ({
          ...prev,
      ...
      // Watch for pointer movement to update coordinates
      ...
      view.when(() => {
        console.log("View ready, adding markers");
        addUserLocationMarkers(view);
        showUserTrail(view);
        // Force initial visibility check
        updateMarkerVisibility(view, view.zoom);
      });
  ...
  useEffect(() => {
    if (viewRef.current) {
      // Update trails whenever showTrail changes
      showUserTrail(viewRef.current);

      // Watch for extent changes
      const extentHandle = viewRef.current.watch("extent", () => {
        if (showTrail) {
          showUserTrail(viewRef.current);
        }
      });
  ...
  }, [showTrail]); // Add showTrail as a dependency
